...
const SoundFamilyTree = ({ originalProject, relatedProjects }) => {
  ...
  useEffect(() => {
    ...
    function buildHierarchy() {
      ...
      allProjects.forEach((project) => {
        if (
          ...
          projectsMap.has(project.parentProjectId)
        ) {
    ...
    // Increase left margin to give more space for parent node text
    const margin = { top: 40, right: 90, bottom: 80, left: 150 };

    // Reduce the effective width to bring nodes closer horizontally
    const width = (dimensions.width - margin.left - margin.right) * 0.7; // Use 70% of available width

    // Dynamically adjust height based on node count to ensure all nodes are visible
    const nodeCount = relatedProjects.length + 1;
    const minHeight = 500;
    const heightPerNode = 70; // Allow more vertical space per node
    const height = Math.max(
      dimensions.height - margin.top - margin.bottom,
      minHeight,
      nodeCount * heightPerNode
    );
    ...
    const treeLayout = d3
      .tree()
      .size([height, width])
      .separation((a, b) => (a.parent === b.parent ? 1.2 : 1.5)); // Adjust node separation
    ...
    nodes
      ...
      .style("stroke", (d) =>
        d.data._id === originalProject._id ? "#FF3D00" : "#333333"
      )
      .style("stroke-width", (d) =>
        d.data._id === originalProject._id ? 3 : 2
      );
      ...
      .text((d) =>
        d.data.title
          ? d.data.title.length > 20
            ? d.data.title.substring(0, 17) + "..."
            : d.data.title
          : "Untitled"
      )
      .style("font-size", "12px")
      .style("font-weight", (d) =>
        d.data._id === originalProject._id ? "bold" : "normal"
      )
      .style("fill", (d) =>
        d.data._id === originalProject._id ? "#000" : "#333"
      );
    ...
    nodes
      ...
      .style("text-anchor", (d) => (d.children ? "end" : "start"))
      .text((d) =>
        d.data.creator && d.data.creator.name ? `by ${d.data.creator.name}` : ""
      )
    ...
    const handleResize = () => {
      ...
      if (container) {
        setDimensions({
          ...
          height: Math.max(500, container.clientWidth * 0.6), // Increased height ratio
        });
      }
    };
